...
sr.reveal('.home__img, .about__subtitle, .about__text , .about__list, .skills__img',{delay: 400}); /* Added .about__text here */
...
setInterval(() => {
  ...
  darkmode.onclick = () => {
      if(darkmode.classList.contains('bx-moon')){
        ...
        // Function to animate on page load
        document.addEventListener("DOMContentLoaded", () => {
          const items = document.querySelectorAll(".about__list li");

          items.forEach((item, index) => {
              setTimeout(() => {
                  item.classList.add("show");
              }, index * 300); // Staggered effect (300ms delay per item)
          });
      });
      ...
      document.querySelectorAll(".about__list li").forEach((li) => {
          observer.observe(li);
      });
